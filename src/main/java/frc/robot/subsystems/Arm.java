...
public class Arm extends SubsystemParent implements Reportable {
  private final TalonFX armMotor;

  private double desiredPosition = 0.0;
  private double maxDegrees = 90.0;
  ...
  public void setMotorConfigs(){
        ...
        StatusCode statusCode = armMotor.getConfigurator().apply(config);
        if (!statusCode.isOK()){
  ...
  public void periodic() {
    ...
    if (getPosition() <= minDegrees){
      desiredPosition = minDegrees;
    } else if(getPosition() >= maxDegrees) {
      desiredPosition = maxDegrees;
    }
    armMotor.setControl(motionMagicVoltage.withPosition(desiredPosition));
  }
  ...
  public void setEnabled(boolean enabled){
    ...
    if(!enabled){
      armMotor.setControl(neutralRequest);
    } 
  }
